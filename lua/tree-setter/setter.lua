...
function Setter.set_character(bufnr, line_num, end_column, character)
	...
	if character == ";" and trimmed_line ~= "" and trimmed_line:sub(-1) == ")" then
		vim.api.nvim_buf_set_lines(0, line_num, line_num + 2, false, { line .. character, indent_fix })
		return
	end
	...
	if character == "=" then
		...
		local updated_line = line:sub(1, end_column) .. character .. line:sub(end_column)
